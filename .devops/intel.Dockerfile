...
RUN if [ "${GGML_SYCL_F16}" = "ON" ]; then \
    ...
    cmake -B build -DGGML_NATIVE=OFF -DGGML_SYCL=ON -DCMAKE_C_COMPILER=icx -DCMAKE_CXX_COMPILER=icpx -DGGML_BACKEND_DL=ON -DGGML_CPU_ALL_VARIANTS=ON -DLLAMA_BUILD_TESTS=OFF ${OPT_SYCL_F16} -DCMAKE_CXX_FLAGS="-DGGML_SYCL_Q4K_DMMV_WARP=16 -DGGML_SYCL_Q4K_DMMV_ESIMD=1 -DGGML_SYCL_Q6K_DMMV_ESIMD=1" && \
